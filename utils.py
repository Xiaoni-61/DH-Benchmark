...
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, \
    FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    elif partition == "noniid-labeldir":  # Distribution-based label imbalance
        min_size = 0
        min_require_size = 10
        K = 10
...
def compute_local_test_accuracy(model, dataloader, data_distribution, device="cpu"):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (x, target) in enumerate(dataloader):
            x, target = x.to(device, dtype=torch.float32), target.to(device, dtype=torch.int64)
            out = model(x)
...
def get_KRI(args):
    if args.dataset == 'cifar10':
        return 4, 1000, 3 * 32 * 32
    elif args.dataset == 'mnist' or args.dataset == 'fmnist':
        raceK, raceR, IN = 6, 800, 28 * 28
        return raceK, raceR, IN
    elif args.dataset == 'femnist':
        raceK, raceR, IN = 10, 2735, 28 * 28
        return raceK, raceR, IN
...
def get_dataloader(args, dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0,
                   dataidxss=None):
    if dataset in (
        ...
        if dataset == "tinyimagenet":
            ...
            train_ds = dl_obj(datadir, train=True, download=True)
            test_ds = dl_obj(datadir, train=False, transform=transform_test, download=True)

        if dataset == 'svhn':
            X_train = train_ds.data.transpose((0, 2, 3, 1))
            # test_ds.data = test_ds.data.transpose((0, 2, 3, 1))
            train_image = np.array(X_train)
        else:
            train_image = np.array(train_ds.data)

        if dataset in ('a9a', 'femnist', 'rcv1', 'covtype'):
            train_label = np.array(train_ds.targets)
        else:
            train_label = np.array(train_ds.target)

        train_dataloaders = []
        val_dataloaders = []
        for i in range(args.n_parties):
            train_idxs = dataidxss[i][:int(0.8 * len(dataidxss[i]))]
            val_idxs = dataidxss[i][int(0.8 * len(dataidxss[i])):]
            train_dataset = Truncated(data=train_image[train_idxs], labels=train_label[train_idxs],
                                      transform=transform_train)
            train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True)
            val_dataset = Truncated(data=train_image[val_idxs], labels=train_label[val_idxs],
                                    transform=transform_train)
    ...
    return train_dataloaders, val_dataloaders, test_dl
